...
你已经注意到模型已经有一些关联的事件,比如`error`和`ajaxError`,但`create/update/destroy`操作的事件回调呢? Spine也已经包含了. 你可以直接这样绑定事件:
